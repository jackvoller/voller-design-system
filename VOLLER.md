...
| Change | Why |
|---|---|
| `--muted` is now `#6E6E6B`; the old `#8A8A8A` is renamed `--muted-dark` | `#8A8A8A` is 3.4:1 on the light field — it fails AA, and it was the colour every eyebrow and caption used |
...
Twelve values. Do not introduce a thirteenth without adding it here first.
...
| `gold` | `#F0C21C` | The action colour. Rationed — see §4. Identical in both fields. |
...
| `muted-dark` | `#8A8A8A` | Secondary text and eyebrows on dark. 5.0:1. |
...
- `#00A000` (the old web green) and `#111827` (the old cool ink) are retired. So is `#EF4444`.
...
| `gold` | `ink` | 10.2 ✓ | The one gold element, both fields |
| `gold` | white | 1.7 ✗ | **Never** |
| `#FCFCF6` | `ink` | 16.8 ✓ | Body copy on light |
| `#FCFCF6` | `muted` | 5.0 ✓ | Captions and eyebrows on light |
| `#FCFCF6` | `#8A8A8A` | 3.4 ✗ | **Never** — use `muted` |
...
| `ink` | `muted-dark` | 5.0 ✓ | Captions and eyebrows on dark |
...
| `ink-raised` | `muted-dark` | 4.6 ✓ | Captions in a dark card |
...
**In product UI**, gold marks *the journey* — the concrete thing the user came to do. Where that is
a set of elements expressing one concept (Riverly's route line, its destination flag, the live boat
and the Go button) the whole set counts as one element. It is not a warning colour, not a highlight
colour, not decoration. If two golds meet, one becomes `green-dark`.

In a header, the wordmark tile spends the gold. **A header containing both the wordmark and a gold
button is wrong.**
...
The house system governs **chrome**. It does not govern **category colour** — the palette an app uses
to distinguish tags, statuses, labels, map markers or domain data.

This is deliberate. Meal Planner needs food-tag colours, studio needs status colours, Riverly needs a
water blue. No single house palette can serve all three, and every attempt to make one is what
produced a 34-utility Tailwind safelist.

### The house governs

Surfaces and fields · hairlines · the type family, scale and weight set · tint · primary and
secondary actions · gold and what it may mark · alert and destructive affordances · radii · the 8px
spacing base and section rhythm · icons, wordmark and app-tile geometry.
...
4. **Two reservations.** A category set may never use `gold` or `alert`, and a category colour may
   never tint a button, a link or a nav item. Those two carry meaning across the whole family.
...
- On dark: cards and rows are `--ink-raised` with `--hairline-dark`; the quiet button becomes a
  `rgba(246,241,227,.20)` outline rather than a tinted wash, which disappears on near-black.
...
- No `#00A000`, `#111827`, `#EF4444`, `#8A8A8A`-on-light, or a light label on `green`.
...
Live component reference: **`voller-design-system-reference.html`**.
...
5. Check it at 29pt.
6. Declare any category colours in the app's own theme file, under §8's four rules.
7. Line it up against the existing icons before shipping. It should be obvious they're siblings
   without being told.
